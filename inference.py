...
def main(args):
    ...
    test_ds = Dataset.from_json(os.path.join("/content/drive/MyDrive/github/모두의말뭉치/","resource/data/nikluge-ea-2023-test.jsonl"))
    # 데이터셋을 변환합니다.
    ...
    def jsonldump(j_list, fname):
        folder_name = args.model_ckpt_path[args.model_ckpt_path.rindex('/'):]
        output_folder = fname + folder_name
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        with open(os.path.join(output_folder,'output.jsonl'), "w", encoding='utf-8') as f:
            for json_data in j_list:
                f.write(json.dumps(json_data, ensure_ascii=False)+'\n')

    j_list = jsonlload(os.path.join("/content/drive/MyDrive/github/모두의말뭉치/","resource/data/nikluge-ea-2023-test.jsonl"))
